...
public class Admin {
    ...
    public void initFilePath() {
        ...
        for (Map.Entry<FilesName, String> entry : fileMap.entrySet()) {
            ...
            String dir = rootDir + File.separator +rootDir2+ File.separator+ rootDir3+File.separator+ entry.getKey().getFileName() + fileType;
            File f = new File(dir);
            if (!f.exists()) {
                try {
                    f.createNewFile();
 ...
 */
    ...
    // TODO: 2022/12/13 need check 
    public boolean delStock(Stock stock) {
        ArrayList<ArrayList<String>> oldData = csv2Array(FilesName.STOCK);
        int index = Integer.MAX_VALUE;
        if (oldData == null) {  //file not exist or file is empty
            return false;
        }
        for (int i = 0; i < oldData.size(); i++) {
            if (oldData.get(i).get(0).equals(stock.getStockName())) {
                index = i;
                break;
            }
        }
        if (index == Integer.MAX_VALUE) { //stock doest not contain the record of this key value
            return false;
        }
        oldData.remove(index);
        String path = fileMap.get(FilesName.STOCK);
        BufferedWriter out = null;
        try {
            out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(path), StandardCharsets.UTF_8));
            for (int i = 0; i < oldData.size(); i++) {
    ...
    public CheckingAccount getCheckingByName(FilesName filesName, String name) {
        ...
        double loanAmount = Double.parseDouble(personInfo.get(++afterIndex));  //currency.type+1
